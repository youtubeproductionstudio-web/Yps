...
-- Native Android/Java bindings (Replaced 'import' to prevent crash on encrypted bytecode)
...
-- Fixed pool capacity with safe task execution
...
local function executeHttp(method, targetUrl, payload)
    ...
    -- Mandatory Connection Release to prevent UI hang & Accessibility Crash
    if conn then
        ...
        run = function()
            while syncRunning do
                local loopSuccess, loopErr = pcall(function()
                    ...
                    -- Heartbeat dispatched safely
                    if now - lastHbTime >= 3 then
                        lastHbTime = now
                        threadPool.execute(Runnable({
                            run = function()
                                if not syncRunning then return end
                    ...
                    if statusOk then
                        if body == "null" or body == "" then
                            uiHandler.post(Runnable({ 
                                run = function() 
                                    if uiCallbacks and uiCallbacks.onRoomClosed then
                                        uiCallbacks.onRoomClosed("Room deleted or closed by host.") 
                                    end
                                end 
                            }))
                            syncRunning = false
                        ...
                        elseif body ~= lastRawBodyCache then
                            ...
                            pcall(function()
                                ...
                                if localKickBanStamp > 0 and liveTime < localKickBanStamp then
                                    uiHandler.post(Runnable({ 
                                        run = function() 
                                            if uiCallbacks and uiCallbacks.onKicked then
                                                uiCallbacks.onKicked() 
                                            end
                                        end 
                                    }))
                                    syncRunning = false
                                ...
                                uiHandler.post(Runnable({
                                    run = function()
                                        if syncRunning and uiCallbacks and uiCallbacks.onDataReceived then
                                            pcall(uiCallbacks.onDataReceived, body, jobj, currentActivePlayersList, getPlayerState)
                                        end
                                    end
                                }))
                            end)
                ...
                -- Interruption-Safe Thread Sleep to avoid JVM / Accessibility crashes
                pcall(function() Thread.sleep(650) end)
            end
        end
    }))
    syncThread.start()
