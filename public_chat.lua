...
function M.show(params)
    ...
    local role = params.role or "user"
    local isAdmin = (role == "admin")

    local chatUrl = "https://card-games-muzammil-munir-default-rtdb.firebaseio.com/chatsPK-Games-01/messages.json"
    ...
    -- DOUBLE BIND KEY LISTENER (Dono par lagaya taake keyboard ke andar se bhi back dabane par dialog aaye)
    ...
    local function displayMessage(sender, text, timeStr, msgKey, replySender, replyText)
                ...
                builder.setItems(opts, DialogInterface.OnClickListener{
                    onClick = function(dialog, which)
                        ...
                        elseif sel == "Delete" then
                            ...
                            SendData.deleteMessage(updateUrl, function(code)
        ...
        chatScroll.post(Runnable{run = function() if _G.AppState.isInChat and chatScroll then chatScroll.fullScroll(ScrollView.FOCUS_DOWN) end end})
    end
