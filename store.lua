...
function store.show(params)
  ...
  -- Audio Playback Function for Successful Purchases
  import "android.media.MediaPlayer"
  local function playPurchaseSound()
    pcall(function()
      local player = MediaPlayer()
      player.setDataSource(tostring(activity.getLuaDir()) .. "/sounds/perchased.mp3")
      player.prepare()
      player.start()
      
      -- Free up memory after the sound finishes playing
